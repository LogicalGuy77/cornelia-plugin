    ...
    const handleRedraft = async () => {
      ...
      try {
        ...
        onGeneratingRedraftsChange((prev) =>
          new Map(prev).set(selectedClause.text, true)
        );
        ...
        if (result) {
          ...
          onRedraftReviewStatesChange((prev) =>
            new Map(prev).set(selectedClause.text, {
              text: result,
    ...
    const handleAcceptRedraft = async (item) => {
      try {
        await Word.run(async (context) => {
          ...
          const foundRange = await searchAndReplaceText(
            context,
            searchText,
            redraftState.text
          );
          if (foundRange) {
            ...
            onRedraftedTextsChange((prev) =>
              new Map(prev).set(item.text, redraftState.text)
            );
      ...
      try {
        await Word.run(async (context) => {
          ...
          const searchResults = context.document.body.search(
            activeCommentItem.text.substring(0, 255)
          );
          context.load(searchResults);
          await context.sync();
          ...
          if (searchResults.items.length > 0) {
            ...
            const comment = searchResults.items[0].insertComment(
              commentContent
            );
            await context.sync();
                ...
                >
                  <List
                    dataSource={[
                      ...
                      ...risky,
                      ...missing,
                    ].filter((item) => redraftedClauses.has(item.text))}
                    renderItem={(item) => (
                      <List.Item
